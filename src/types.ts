...
export type Config = Partial<Record<CountryCode | 'showErrors' | 'includeExternalUrls' | 'mediaFlowProxyUrl' | 'mediaFlowProxyPassword' | 'torboxApiKey', string> & Record<`disableExtractor_${string}`, string> & Record<`excludeResolution_${string}`, string>>;
